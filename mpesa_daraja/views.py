...
class PaymentViewSet(PermissionsPerMethodMixin, viewsets.ModelViewSet):
    ...
    def callback(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            ...
            transactionDate = request.data.get('Body').get('stkCallback').get('CallbackMetadata').get('Item')[3].get('Value')
            transaction_id = request.data.get('Body').get('stkCallback').get('CallbackMetadata').get('Item')[1].get('Value')
            phone_number = request.data.get('Body').get('stkCallback').get('CallbackMetadata').get('Item')[4].get('Value')
